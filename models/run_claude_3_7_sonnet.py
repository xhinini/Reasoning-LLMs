#!/usr/bin/env python3
"""
Script to run Claude-3.7-Sonnet-Thinking model on code generation tasks.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("claude_3_7_evaluation.log"),
...
class ClaudeRunner:
    """Class to run Claude-3.7-Sonnet-Thinking model for reasoning tasks using OpenRouter API."""
    ...
    def __init__(self, reasoning_effort="medium"):
        ...
        logger.info(f"Initialized ClaudeRunner with reasoning_effort: {reasoning_effort}")
    
    def generate_completion(self, prompt: str) -> Dict[str, Any]:
        """Generate completion using Claude-3.7-Sonnet-Thinking via OpenRouter API.
    ...
    def _extract_reasoning_content(self, content: str) -> str:
        ...
        """
        # Check for Claude's thinking format
...
def run_evaluation(task_type, samples_dir, results_dir, reasoning_effort="medium", sample_limit=None, skip=None):
    ...
    runner = ClaudeRunner(reasoning_effort=reasoning_effort)
    ...
    # Define output path
    output_path = os.path.join(results_dir, f"claude_3_7_{task_type}_results.json")
...
def main():
    """Main function to run Claude-3.7-Sonnet-Thinking evaluation on dataset samples."""
    # Parse arguments
    import argparse
    parser = argparse.ArgumentParser(description="Run Claude-3.7-Sonnet-Thinking evaluation on code samples")
    parser.add_argument("--samples_dir", type=str, default="dataset_samples",
                        help="Directory containing dataset samples")
    parser.add_argument("--results_dir", type=str, default="claude_results",
